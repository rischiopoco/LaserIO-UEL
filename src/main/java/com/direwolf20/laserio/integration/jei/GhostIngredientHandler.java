...
public class GhostIngredientHandler<T extends AbstractContainerScreen<?>> implements IGhostIngredientHandler<T> {
    ...
    public <I> List<Target<I>> getTargetsTyped(T gui, ITypedIngredient<I> ingredient, boolean doStart) {
        ...
        for (Slot slot : gui.getMenu().slots) {
                ...
                targets.add(new Target<I>() {
                    ...
                    public void accept(I ingredient) {
                        ...
                        if (gui instanceof FilterCountScreen filterCountGui) {
                            //We do this for continuity between client/server -- not needed in cardItemScreen
                            filterCountGui.getMenu().handler.setStackInSlotSave(slot.index, itemStack);
                        }
            ...
            } else if (ModIntegration.MEKANISM.isLoaded() && ingredient.getIngredient() instanceof ChemicalStack<?>) {
                targets.add(new Target<I>() {
                    @Override
                    public Rect2i getArea() {
                        return bounds;
